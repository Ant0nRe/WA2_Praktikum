...
class ReservierungsitzeDao {
    ...
    create(titel = '', platzid = '', status = '') {
        var sql = 'INSERT INTO Reservierungsitze (titel,platzid,status) VALUES (?,?,?)';
        var statement = this._conn.prepare(sql);
        var params = [titel,platzid,status];
    ...
    update(id, titel = '', platzid = '', status = '') {
        var sql = 'UPDATE Reservierungsitze SET titel=?,platzid=?,status=? WHERE id=?';
        var statement = this._conn.prepare(sql);
        var params = [titel,platzid,status, id];
        var result = statement.run(params);
